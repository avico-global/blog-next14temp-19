...
export default function Footer({ categories, logo, imagePath, blog_list }) {
  ...
  return (
    <FullContainer className="bg-[#111111]/90 py-16 relative">
      <Image src={footer} alt="footer" className="absolute -z-20 top-0 left-0 w-full h-full object-cover" />
        ...
        <div>
          ...
          <div className="flex flex-col gap-6">
            {editorPicks.map((post, index) => (
              <Link href={`/${sanitizeUrl(post?.title)}`} key={index}>
                <div className=" gap-4 group grid grid-cols-3">
                  ...
                  <div className="col-span-2">
                    ...
                    <p className="text-gray-400 text-xs mt-2">{post?.published_at}</p>
                  </div>
                </div>
              </Link>
            ))}
